...
async def help(ctx, *, command_name=None):
    if command_name:
        ...
        if command_name in admin_commands and not (
...
async def slash_help(interaction: discord.Interaction, command: str = None):
    ...
    if command:
        ...
        if command in admin_commands and not (
            ...
            await interaction.response.send_message(
                f"❌ You don't have permission to use the `{command}` command!", ephemeral=True)
            return
        ...
        if command == "secret":
            ...
            if not role or role not in interaction.user.roles:
                await interaction.response.send_message(
                    f"❌ You don't have the required role to use the `{command}` command!", ephemeral=True)
                return
